...
# Check if port is available by trying to bind it locally.
is_port_available() {
    local port=$1
    log_info "Checking port $port availability..." >&2

    if ! command -v python3 >/dev/null 2>&1; then
        log_error "python3 is required to check port availability" >&2
        return 1
    fi
...
PY
    then
        log_info "python bind detected port $port is free" >&2
        return 0
    else
        log_info "python bind detected port $port is in use" >&2
        return 1
    fi
}
